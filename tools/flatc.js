...
flatc.Generator = class {
    ...
    _buildTable(type) {
        ...
        this._builder.indent();
            ...
            this._builder.add('');
            this._builder.add('static decodeText(json) {');
            this._builder.indent();
                this._builder.add('const $ = new ' + name + '();');
                for (const field of type.fields.values()) {
                    if (field.repeated) {
                        if (field.type instanceof flatc.PrimitiveType) {
                            ...
                            const fieldType = '$root.' + field.type.parent.name + '.' + field.type.name;
                            this._builder.add('$.' + field.name + ' = Array.isArray(json.' + field.name + ') ? json.' + field.name + '.map((obj) => ' + fieldType + '.decodeText(obj)) : [];');
                        }
                    }
                    else {
                        if (field.type instanceof flatc.Enum) {
                            ...
                            const unionType = '$root.' + field.type.parent.name + '.' + field.type.name;
                            this._builder.add('$.' + field.name + ' = ' + unionType + '.decodeText(json.' + field.name + ', json.' + field.name + '_type' + ');');
                        }
    ...
    _buildUnion(type) {
        ...
        this._builder.indent();
            ...
            this._builder.add('');
            this._builder.add('static decodeText(json, type) {');
            this._builder.indent();
                this._builder.add('switch (type) {');
                this._builder.indent();
                    for (const pair of type.values) {
                        const valueType = '$root.' + pair[1].parent.name + '.' + pair[1].name;
                        this._builder.add('case \'' + pair[1].name + '\': return ' + valueType + '.decodeText(json);');
                    }
                this._builder.outdent();
                this._builder.add('}');
                this._builder.add('return undefined;');
